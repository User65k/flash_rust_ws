...
pub async fn check_is_authorized(auth: &Authenticatoin, req: &Request<Body>) -> Result<Option<Response<Body>>, IoError> {
    match auth {
...
mod tests {
    ...
    fn get_map() {
        ...
        assert_eq!(
            m.get(b"username".as_ref()).unwrap(),
            &b"dani".as_ref());
        assert_eq!(m.get(b"realm".as_ref()).unwrap(), &&b"a realm"[..]);
        assert_eq!(m.get(b"qop".as_ref()).unwrap(), &&b"auth"[..]);
        assert_eq!(m.get(b"nonce".as_ref()).unwrap(), &&b"dcd98b7102dd2f0e8b11d0f600bfb0c093"[..]);
        assert_eq!(m.get(b"uri".as_ref()).unwrap(), &&b"/favicon.ico"[..]);
        assert_eq!(m.get(b"algorithm".as_ref()).unwrap(), &&b"MD5-sess"[..]);
        assert_eq!(m.get(b"response".as_ref()).unwrap(), &&b"f02dcc493b488cc25d503c15765a3005"[..]);
        assert_eq!(m.get(b"nc".as_ref()).unwrap(), &&b"00000002"[..]);
        assert_eq!(m.get(b"cnonce".as_ref()).unwrap(), &&b"30d60c5e1664d9cb"[..]);
