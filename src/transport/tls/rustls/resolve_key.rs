use super::{load_certs, load_private_key, KeySet};
...
impl ResolveServerCert {
    ...
    where
        F: FnOnce(Option<&CertifiedKey>) -> T,
    {
...
impl ResolveServerCert {
    ...
    pub fn add(&mut self, sni: Option<&str>, keyset: &[KeySet]) -> Result<(), Error> {
        let mut ident = CertKeys {
            rsa: None,
            ec: None,
            ed: None,
        };
...
impl ResolvesServerCert for ResolveServerCert {
    fn resolve(&self, client_hello: ClientHello) -> Option<Arc<CertifiedKey>> {
        ...
        if client_hello
            .alpn()
            .and_then(|mut f| f.find(|alpn| *alpn == ACME_TLS_ALPN_NAME))
            .is_some()
        {
